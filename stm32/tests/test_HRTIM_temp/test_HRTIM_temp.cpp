...
void setup() {
    ...
    const uint16_t phases[5] = {0, 5461, 10923, 0, 0};
...
/*
 * HRTIM1_CHA1 -> PA8   -> AF13
 * HRTIM1_CHA2 -> PA9   -> AF13
 * HRTIM1_CHB1 -> PA10  -> AF13
 * HRTIM1_CHB2 -> PA11  -> AF13
 * HRTIM1_CHC1 -> PB12  -> AF13
 * HRTIM1_CHC2 -> PA13  -> AF13
 * HRTIM1_CHD1 -> PB14  -> AF13
 * HRTIM1_CHD2 -> PA15  -> AF13
 * HRTIM1_CHE1 -> PC8   -> AF3
 * HRTIM1_CHE2 -> PC9   -> AF3
 */
...
/*
    pinMode(7,  ALTERNATE);
    pinMode(2,  ALTERNATE);
    pinMode(22, ALTERNATE);
    pinMode(23, ALTERNATE);
    pinMode(24, ALTERNATE);
*/
    /* Turn on LED to signal success */
    pinMode(13, OUTPUT);
    digitalWrite(13, 1);
}
...
void loop() {
    ...
    count++;
    HRTIM1->sTimerxRegs[0].CMP1xR = (uint32_t)(period - lut_g[count]);
    HRTIM1->sTimerxRegs[1].CMP1xR = (uint32_t)(period - lut_r[count]);
    HRTIM1->sTimerxRegs[2].CMP1xR = (uint32_t)(period - lut_b[count]);
...
#if 0
    static uint16_t duties[3];
    static uint16_t count = 0;

    if (count++ == period - limits[prescaler].min)
        count = 0;

    for (int i = 0; i < 3; i++)
        HRTIM1->sTimerxRegs[i].CMP1xR = (uint32_t)(period - count);
...
#if 0
    /* Set and reset TA1 and TB1 by software */

    HRTIM1->sTimerxRegs[0].SETx1R = HRTIM_SET1R_SST;
    HRTIM1->sTimerxRegs[1].SETx1R = HRTIM_SET1R_SST;

    HRTIM1->sTimerxRegs[0].RSTx1R = HRTIM_RST1R_SRT;
    HRTIM1->sTimerxRegs[1].RSTx1R = HRTIM_RST1R_SRT;
#endif
}
